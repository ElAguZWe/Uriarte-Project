...
passport.use(new Elocal.Strategy(
    (username, password, done) => {
        ...
        const dbBusqueda = db.query('SELECT * FROM `registro` WHERE nombreUsuario = ?', [username], (err, resultados) => { //Discriminar nombre de usuario entre mayusculas y minusculas -- PENDIENTE
            ...
            bcrypt.compare(password, usuario.contrasenia, (error, coinciden) => {
                if (error) {
                    return done(error);
                }
                if (coinciden) {
...
passport.deserializeUser((id, done) => {
    db.query('SELECT * FROM registro WHERE idUsuario = ?', [id], (err, resultados) => {
        if (err) {
            return done(err);
        }
        if (!resultados || resultados.length === 0) {
